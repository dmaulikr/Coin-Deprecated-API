...
- (void)renderSliderBubbleView{
    ...
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, 300, viewMax);
    [self setViewWithSliderFrame:CGRectMake(_leftMargin, _topMargin, 300 - (_leftMargin*2), kSliderHeight)];
}
...
//    [_slider setMinimumTrackImage:_maxTrackImage forState:UIControlStateNormal];
    ...
    _valueLabel.text = [NSString stringWithFormat:@"%.2f %%",(float)self.slider.value];
...
-(void)slierValueChanged:(id)sender{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(getSliderDidChangeValue:)]){
        [self.delegate getSliderDidChangeValue:(float)self.slider.value];
    }
}
...
-(void)sliderTouchEnd:(id)sender{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(getSliderDidEndChangeValue:)]){
        [self.delegate getSliderDidEndChangeValue:(float)self.slider.value];
    }
}
